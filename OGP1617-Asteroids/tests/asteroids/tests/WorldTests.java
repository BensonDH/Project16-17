...
public class WorldTests {
	...
	// -*-*- evolve tests -*-*-
	@Test
	public void evolveWithShipCollisionTest(){
		World testWorld = new World(150, 150);
		...
		// Helper method test
		Collision firstCollision = testWorld.getFirstCollision();
		...
		Vector oldVelocity2 = ship2.getVelocity();
		Vector oldVelocity1 = ship1.getVelocity();
		Vector deltaV = oldVelocity2.subtract(oldVelocity1);
		...
		testWorld.evolve((70-10*Math.sqrt(2))/15.0, null);
			...
			// Velocity
			double mi = ship1.getTotalMass();
			double mj = ship2.getTotalMass();
			double sigma = ship1.getRadius()+ship2.getRadius();
			Vector deltaX = ship2.getPosition().subtract(ship1.getPosition());

			
			double J = (2*mi*mj*deltaX.dot(deltaV))/(sigma*(mi+mj));
			double JxAndy = (J*deltaX.getX())/sigma;
			
			assertEquals(oldVelocity1.getX()+JxAndy/mi, ship1.getVelocity().getX(), EPSILON);
			assertEquals(oldVelocity1.getY()+JxAndy/mi, ship1.getVelocity().getY(), EPSILON);
			
			assertEquals(oldVelocity2.getX()-JxAndy/mi, ship2.getVelocity().getX(), EPSILON);
			assertEquals(oldVelocity2.getY()-JxAndy/mi, ship2.getVelocity().getY(), EPSILON);
	}
	
	@Test
	public void evolveWithBulletShipCollisionTest(){
		World testWorld = new World(150, 150);
		
		Bullet testBullet = new Bullet(50, 50, 5, 5, 10);
		testWorld.addEntity(testBullet);
		Ship testShip = new Ship(120, 120, -10, -10, 10, 0);
		testWorld.addEntity(testShip);
		// Both entities have the same position, velocity and radius as the entities
		// in evolveWithShipCollisionTest(), hence testing the time and position until collision
		// would be pointless.
		
		// A ship will collide with a bullet that was not fired by this ship
		testWorld.evolve((70-10*Math.sqrt(2))/15.0, null);
		
		// Both bullet and ship have to be terminated
		assertTrue(testBullet.isDead());
		assertTrue(testShip.isDead());
		
		assertFalse(testWorld.isInWorld(testBullet));
		assertFalse(testWorld.isInWorld(testShip));
	}
	
	@Test
	public void evolveWithBulletSourceShipCollisionTest(){
		// A ship will collide with a bullet that was fired by this ship
		World testWorld = new World(150, 150);
		
		Bullet testBullet = new Bullet(50, 50, 5, 5, 10);
		testWorld.addEntity(testBullet);
		Ship testShip = new Ship(120, 120, -10, -10, 15, 0);
		testWorld.addEntity(testShip);
		testBullet.setSourceShip(testShip);
		
		// getTimeToCollision test
		double time = testBullet.getTimeToCollision(testShip);
		assertEquals(14/3.0-(5*Math.sqrt(2))/6.0, time, EPSILON);
		
		testWorld.evolve(14/3.0-(5*Math.sqrt(2))/6.0, null);
		
		// Both bullet and ship must still live
		assertFalse(testBullet.isDead());
		assertFalse(testShip.isDead());
		
		// The bullet has to be removed from the world
		assertFalse(testWorld.isInWorld(testBullet));
		assertNull(testBullet.getWorld());
		assertTrue(testWorld.isInWorld(testShip));
		
		// The bullet has to be reloaded on the ship
		assertTrue(testShip.getBullets().contains(testBullet));
		assertTrue(testBullet.getShip() == testShip);
	}
	
	@Test
	public void evolveWithBoundaryCollisionTest(){
		// A bullet will collide with a boundary
		World testWorld = new World(150, 150);
		
		Bullet testBullet = new Bullet(11, 10, -5, 0, 1);
		testWorld.addEntity(testBullet);
		
		// getTimeToCollisionTest
		double time = testWorld.getTimeToCollisionWithBoundaries(testBullet);
		assertEquals(2, time, EPSILON);
		
		testWorld.evolve(2, null);
		
		// The bullet's X-component of velocity has to be flipped
		assertEquals(5, testBullet.getVelocity().getX(), EPSILON);
		
		// The bullet's bounce counter has to be added with 1
		assertEquals(testBullet.getNbTimesBounced(), 1);
		
		// A bullet will collide with a boundary but it will die because it has bounced off
		// a boundary to many times.
		testWorld = new World(150, 150);
		
		testBullet = new Bullet(11, 10, -5, 0, 1);
		testBullet.setNbTimesBounced(testBullet.getMaxTimesBounced());
		testWorld.addEntity(testBullet);
		
		// getTimeToCollisionTest
		time = testWorld.getTimeToCollisionWithBoundaries(testBullet);
		assertEquals(2, time, EPSILON);
		
		testWorld.evolve(2, null);
		
		// The bullet has to be terminated
		assertTrue(testBullet.isDead());
		assertFalse(testWorld.isInWorld(testBullet));
		assertNull(testBullet.getWorld());
	}
